...
export const SHALLOW_DECK: readonly DeckEntry[] = [
	...
	{ kind: "scale", count: 2 }, // 防御4。まんぜう軍（レベル4〜）・毒矢の罠（レベル4〜）の ちから下げを防ぐ、を教える
	{ kind: "steelsh", count: 1 }, // 防御6。鋼鉄の盾の位置。いちばん強い盾で 1枚だけ
	// 矢 5（ちょっと は 矢が多め：7.8%）
	{ kind: "a_wood", count: 3 }, // 各層に1束。寝落ち民・自演くんを 離れて削る
	...
	{ kind: "h_antidote", count: 3 }, // ちょっと では 草の中で倍の率。まんぜう軍・毒矢の罠・チギュリパンの あと始末。各層に1つ
	{ kind: "h_might", count: 2 }, // ちからの種
	{ kind: "h_blink", count: 2 }, // ルーラ草。逃げ道
	{ kind: "h_fire", count: 2 }, // 火炎草。飲めば 65〜75 で 自演くん（HP27）も一撃
...
/**
 * はじめの10階の 階 → 本編の何階ぶんか。B1〜4 は罠なし（レベル2 まで）、B5 から罠、B10 は 自演くんの出はじめ。
 */
export const SHALLOW_LEVEL: readonly number[] = [
	...
	4, // B6：ピッチャー・まんぜう軍・コピペ が 一度に来る。毒矢の罠も
	4, // B7：同じ顔ぶれを もう1階（3種を 1階で覚えるのは 多い）
	5, // B8：ゾンJ民（起き上がる）。弱い敵が 抜ける。錆び・地雷の罠も
	6, // B9：拾い画UFO・風吹けば名無し（吹きとばし）
	7, // B10：自演くん（本編の キメラ。B7〜）。ちょっと の B10 も 本編の キメラ・きめんどうし・おばけキノコ
];
...
export const DEEP_DECK: readonly DeckEntry[] = [
	...
	{ kind: "h_antidote", count: 6 }, // 毒草8・チギュリパン5・まんぜう軍・毒矢の罠 が 多いので 本編3 → 6
	{ kind: "h_fire", count: 2 }, // もっと では 消えた 火炎草。1つ減らして 本編より まれに
	{ kind: "h_sight", count: 3 }, // 罠7〜9 と 透明あぼーん（見えない。レベル15〜）に。本編2 → 3
	// スレ 57（未識別）
