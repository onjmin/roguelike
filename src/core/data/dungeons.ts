...
export const SHALLOW_DECK: readonly DeckEntry[] = [
	...
	// 食べもの 6（ちょっと：パン・大きいパン・くさったパン が 1:1:1、全体の 9.4%）
	...
	{ kind: "h_antidote", count: 3 }, // ちょっと では 草の中で倍の率。毒カボチャ・毒矢の罠・くさったパンの あと始末。各層に1つ
...
export const SHALLOW_LEVEL: readonly number[] = [
	0, // [0] 使わない
	1, // B1：とうすこ・ひとだま・迷いコウモリ・フナムシ。罠なし
	1, // B2：同じ顔ぶれで 慣れる
	2, // B3：kskボット・寝落ち民（眠りの呪文）が 加わる
	2, // B4：罠なし最後の階（レベル2 まで 罠 0）
	3, // B5：罠が出はじめる（1〜3個。トラバサミ・眠り・転び・矢・ワープ・落とし穴）。とうすこが 消える
...
/**
 * もっと深い30階の山札（210枚。本編 ×1.5 から 大きなパン・不食の指輪を抜き、マイナスの品を ふやし、
 * いちばん強い品は 本編と同じ数のまま）。食べものは 始めの200% ＋ 1100%（1階 433ターン）。
 * スレ（パン）・草を飲む（+5%）・革の盾で しのぐ。
 */
export const DEEP_DECK: readonly DeckEntry[] = [
	...
	{ kind: "scale", count: 2 }, // 毒で ちからを下げられない（毒草8・くさったパン5 の迷宮で 価値が上がる）
	...
	{ kind: "h_antidote", count: 6 }, // 毒草8・くさったパン5・毒カボチャ・毒矢の罠 が 多いので 本編3 → 6
	...
	{ kind: "s_bread", count: 3 }, // もっと の パンの巻物。大きなパンが 床に無いぶんの 逃げ道。各層に1枚
	...
	// 食べもの 17（大きなパンは 入れない。もっと の パン:くさったパン ≒ 2:1）
